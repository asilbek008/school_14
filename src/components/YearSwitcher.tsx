...
type Props = { lang: string; years: number[]; current: number; format: string; label: string; currentLabel: string; variant?: "bar" | "menu" };

const name = (format: string, y: number) => format.replace("{from}", String(y)).replace("{to}", String(y + 1));

/**
 * "2026–2027 o‘quv yili ▾" in the top bar: lists the school years; each opens its year page, which also
 ...
 */
export default function YearSwitcher({ lang, years, current, format, label, currentLabel, variant = "bar" }: Props) {
  ...
  return (
    <div ref={box} className="relative">
      <button
        ...
        className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 transition-colors hover:bg-white/10 hover:text-white ${picked !== current ? "bg-gold/20 font-semibold text-[#f4c779]" : ""}`}
      >
        {name(format, picked)}
      ...
      {open && (
        <div className="surface absolute left-0 top-full z-50 mt-2 w-60 animate-fade-in rounded-2xl bg-white p-2 text-slate-900 shadow-xl [animation-duration:0.15s]">
