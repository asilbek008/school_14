...
/**
 * The class's pupils under the parallel classes: counts, a boys/girls bar and the list — short names only ("Aliyev A.",
...
export default function ClassPupils({ classId, t }: { classId: number; t: Dictionary["timetable"]["pupils"] }) {
  ...
  const shown = list ? (open ? list : list.slice(0, FIRST)) : [];

  return (
    <section aria-labelledby="class-pupils" className="mt-5 rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h2 id="class-pupils" className="text-lg font-bold text-slate-900">
            {t.title}
            {list && <span className="ml-2 rounded-full bg-navy px-2.5 py-0.5 align-middle text-sm font-bold text-white">{list.length}</span>}
          </h2>
          <p className="mt-0.5 text-[13px] text-slate-500">{list ? t.note : t.loading}</p>
        </div>
        {list && (
      ...
      {list ? (
        <>
          <ol className="mt-5 grid grid-cols-1 gap-2 min-[420px]:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
            {shown.map((p, i) => (
          ...
          {list.length > FIRST && (
            <button
              ...
              aria-expanded={open}
              className="press mt-4 rounded-full bg-brand-soft px-4 py-2 text-sm font-bold text-brand-deep hover:bg-brand hover:text-white"
            >
              {open ? t.hide : fill(t.show, { n: list.length })}
            </button>
          )}
        </>
      ) : (
        <div className="mt-5 grid grid-cols-2 gap-2 md:grid-cols-4" aria-hidden="true">
          {Array.from({ length: 8 }, (_, i) => (
            <span key={i} className="h-[52px] animate-pulse rounded-xl bg-slate-100" />
          ))}
        </div>
      )}
    </section>
