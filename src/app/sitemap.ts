...
import { getAlbums, getClasses, getClubs, getNews, getPrograms, getSchoolYears, getStaff } from "@/lib/content";
import { siteUrl } from "@/lib/school";

export const revalidate = 3600;

const pages = ["", "/about", "/admissions", "/timetable", "/schedule", "/calendar", "/staff", "/news", "/events", "/programs", "/clubs", "/gallery", "/faq", "/documents", "/contact"];
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const [news, staff, clubs, programs, albums, classes, years] = await Promise.all([
    getNews(),
    getStaff(),
    getClubs(),
    getPrograms(),
